...
public class WopiFilterCloud implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (uri.endsWith("/contents")) { // GetFile ：返回文件流
			uri = uri.replace("/contents", "");
			docId = "{"+ uri.substring(uri.lastIndexOf("files/")+6, uri.lastIndexOf("/")-1) + "}";//"{"+fileUri.substring(fileUri.lastIndexOf("files")+6)+"}";
			System.out.println(">>>get contents, doc id ："+docId);
			realm = uri.substring(uri.lastIndexOf("/")+1);
			System.out.println(">>>get contents, realm ："+docId);
			OfficeFileContentDTO content = officeOnline.getFile(docId);
			getFile(content, httpResponse);

		} else { // CheckFileInfo ：返回json
			
			docId = "{" + uri.substring(uri.lastIndexOf("files/")+6, uri.lastIndexOf("/")-1) + "}";//"{"+fileUri.substring(fileUri.lastIndexOf("files")+6,fileUri.lastIndexOf("/"))+"}";
			System.out.println(">>>checkFileInfo, doc id：" + docId);
			realm = uri.substring(uri.lastIndexOf("/")+1);
			System.out.println(">>>get contents, realm ："+docId);
